...
# predefined examples
crypto_examples = ["BTC", "ETH", "DOGE", "SOL"]
us_examples = ["AAPL", "NVDA", "TSLA", "MSFT"]
tw_examples = ["2330", "2454", "2303"]  # numbers will be .TW

def smart_parse_input(s, market):
    s = s.strip().upper()
    if market == "台股":
        if "." in s or "-" in s:
            return s
        ...
        return s  # assume US ticker like AAPL
    else:  # crypto
        if "-" in s or "." in s: return s
        return f"{s}-USD"

# quick pick buttons
st.sidebar.markdown("### 常見標的")
if market == "加密貨幣":
    for c in crypto_examples:
        if st.sidebar.button(c, key=f"pick_{c}"):
            st.session_state.chart_symbol = smart_parse_input(c, market)
            st.rerun()
elif market == "美股":
    for c in us_examples:
        if st.sidebar.button(c, key=f"pick_{c}"):
            st.session_state.chart_symbol = smart_parse_input(c, market)
            st.rerun()
else:
    for c in tw_examples:
        if st.sidebar.button(c, key=f"pick_{c}"):
            st.session_state.chart_symbol = smart_parse_input(c, market)
            st.rerun()
...
# manual input
def update_symbol():
    st.session_state.chart_symbol = smart_parse_input(st.session_state.symbol_input, st.session_state.market)
