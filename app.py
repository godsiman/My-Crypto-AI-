...
# --- Auto refresh (keeps app running, but heavy calls are cached) ---
...
# --- Cached price fetch to reduce yfinance calls ---
@st.cache_data(ttl=30)
def fetch_price_cached(sym):
    try:
    ...
    for i, order in enumerate(list(st.session_state.pending_orders)):
        try:
            ...
            if triggered:
                ...
                st.toast(f"⚡ 掛單成交！{symbol} {order['type']} @ {fmt_price(current_price)}")
        except Exception as e:
            # ignore single order exceptions
            print("order check error", e)
    # remove triggered orders (by original indices)
...
def calculate_supertrend(df, period=9, multiplier=3.9):
    ...
    for i in range(1, len(df)):
        if bu[i] < fu[i - 1] or close_values[i - 1] > fu[i - 1]:
            fu[i] = bu[i]
        else:
            fu[i] = fu[i - 1]
        if bl[i] > fl[i - 1] or close_values[i - 1] < fl[i - 1]:
            fl[i] = bl[i]
        else:
            fl[i] = fl[i - 1]
        if direction[i - 1] == 1:
            if close_values[i] <= fl[i]:
                direction[i] = -1
                st_val[i] = fu[i]
            else:
                direction[i] = 1
...
# --- Chart data fetch (cached) ---
@st.cache_data(ttl=60)
def get_chart_data_cached(symbol, interval_ui):
    ...
    if interval_ui == "15分鐘":
        ...
        # yfinance doesn't always support 4h directly; fetch 1h and resample
        period, interval = "6mo", "1h"
    else:
    ...
    try:
        ...
        # ensure timezone-naive index for plotting
        if isinstance(df.index, pd.DatetimeIndex):
            df.index = df.index.tz_localize(None)
        return df
    except Exception as e:
        print("get_chart_data_cached error", e)
        return None

def get_chart_data(symbol, interval_ui):
    return get_chart_data_cached(symbol, interval_ui)

# --- Backtest engine (Next Open) cached ---
@st.cache_data(ttl=600)
def run_backtest_supertrend_cached(symbol, interval_ui, initial_capital=10000):
    df = get_chart_data(symbol, interval_ui)
    if df is None or len(df) < 120:
        return None, None
    return run_backtest_supertrend(df, initial_capital)
...
def get_supertrend_strategy(symbol, current_interval_ui):
    ...
    if last['ST_Direction'] == 1:
        ...
        score -= 1
    if last['Close'] > last['EMA52']:
        score += 1
    else:
        score -= 1
    if "藍" in qqe_status:
        score += 1
    elif "紅" in qqe_status:
        score -= 1
    direction = "觀望"
...
def on_select_change():
    ...
    if st.session_state.market == "加密貨幣" and "-" not in new_sym and "USD" not in new_sym:
        # keep the same format as yfinance expects
        new_sym += "-USD"
...
# --- Position management dialog (Streamlit dialog decorator used earlier) ---
@st.dialog("⚡ 倉位管理")
def manage_position_dialog(i, pos, current_price):
    ...
    with tab_tpsl:
        ...
        if mode == "價格":
            ...
            d = 1 if pos.get('type') == 'Long' else -1
            if roe_tp > 0:
                new_tp = entry * (1 + (roe_tp / 100.0) / lev * d)
            if roe_sl > 0:
                new_sl = entry * (1 - (roe_sl / 100.0) / lev * d)
...
def close_position(pos_index, percentage, reason, exit_price):
    ...
    try:
        ...
        if percentage == 100:
            ...
            st.session_state.positions[pos_index]['margin'] = float(pos.get('margin', 0)) - close_margin
        save_data()
    except Exception as e:
        st.error(f"平倉失敗: {e}")
...
# --- Auto-monitor positions for TP/SL and liquidation ---
def monitor_positions_and_close(symbol, current_price):
    if current_price is None:
        return
    to_close = []
    for i, pos in enumerate(list(st.session_state.positions)):
        try:
            ...
            lev = float(pos.get('lev') or 1)
            d = 1 if pos.get('type') == 'Long' else -1
            # Check TP/SL
            if tp > 0 and ((d == 1 and current_price >= tp) or (d == -1 and current_price <= tp)):
                # full close
                close_position(i, 100, "TP 達成", current_price)
                st.toast(f"✅ {pos['symbol']} TP 達成 平倉 @ {fmt_price(current_price)}")
            elif sl > 0 and ((d == 1 and current_price <= sl) or (d == -1 and current_price >= sl)):
                # full close
                close_position(i, 100, "SL 觸發", current_price)
                st.toast(f"⚠️ {pos['symbol']} SL 觸發 平倉 @ {fmt_price(current_price)}")
            else:
                # check liquidation extreme (ROE <= -100%)
                margin = float(pos.get('margin', 0))
                if margin > 0:
                    # compute unrealized pnl
                    pnl = margin * (((current_price - entry) / entry) * lev * d)
                    roe_pct = (pnl / margin) * 100
                    if roe_pct <= -100.0:
                        close_position(i, 100, "💀 爆倉 (-100%)", current_price)
                        st.toast(f"⚠️ {pos['symbol']} 已爆倉！保證金歸零")
        except Exception as e:
            print("monitor error", e)
    # save after monitoring
    save_data()
...
if ai_res:
    ...
    df_chart = ai_res['df']
    # auto-check pending orders and positions
    check_pending_orders(symbol, curr_price)
    monitor_positions_and_close(symbol, curr_price)
    ...
    for pos in st.session_state.positions:
        try:
            ...
            pass
    colors = ['#2962FF' if h > 0 else '#FF1744' for h in df_chart['Hist'].fillna(0)]
        ...
        with col_ctrl:
            ...
            with st.expander("進階 (止盈止損)", expanded=True):
                ...
                if mode == "價格":
                    ...
                    if roe_tp > 0:
                        t_tp = curr_price * (1 + (roe_tp / 100.0) / lev * d)
                    if roe_sl > 0:
                        t_sl = curr_price * (1 - (roe_sl / 100.0) / lev * d)
            ...
            if st.button("🚀 下單 / 掛單", type="primary", use_container_width=True):
                ...
                if mode == "ROE %":
                    d = 1 if "多" in trade_type else -1
                    if roe_tp > 0:
                        t_tp = final_entry * (1 + (roe_tp / 100.0) / lev * d)
                    if roe_sl > 0:
                        t_sl = final_entry * (1 - (roe_sl / 100.0) / lev * d)
                if amt > available:
                    st.error(f"可用餘額不足！ (可用: ${available:.2f})")
                else:
                    new_pos = {
                        "symbol": symbol, "type": "Long" if "多" in trade_type else "Short",
                        "entry": float(final_entry), "lev": lev, "margin": float(amt), "tp": float(t_tp), "sl": float(t_sl),
    ...
    with tab_orders:
        ...
        if not st.session_state.positions:
            ...
            for i, pos in enumerate(list(st.session_state.positions)):
                try:
                    ...
                    pnl = pos['margin'] * (((p_cur - pos['entry']) / pos['entry']) * pos['lev'] * d) if p_cur is not None else 0
                    roe_pct = (pnl / pos['margin']) * 100 if pos['margin'] > 0 else 0
                    if roe_pct <= -100.0:
                        close_position(i, 100, "💀 爆倉 (-100%)", p_cur or pos['entry'])
                    ...
                    if c_mng.button("⚙️", key=f"mng_{i}"):
                        manage_position_dialog(i, pos, p_cur or pos['entry'])
                    st.divider()
                except Exception as e:
                    print("持倉顯示錯誤", e)
        ...
        if not st.session_state.pending_orders:
            ...
            for i, ord in enumerate(list(st.session_state.pending_orders)):
                o_sym = ord['symbol']
    ...
    with tab_backtest:
        ...
        if st.button("🚀 開始回測"):
            with st.spinner("正在模擬真實交易情境..."):
                eq_curve, trades_log = run_backtest_supertrend_cached(symbol, interval_ui, 10000)
